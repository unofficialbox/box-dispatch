...
export function ChoosePage({ templates, selectedTemplateID, selectedComponents, assembling, notice, onTemplateChange, onToggleSalesforce, onAssemble }: { templates: SolutionTemplate[]; selectedTemplateID: string; selectedComponents: string[]; assembling: boolean; notice: string; onTemplateChange: (templateID: string) => void; onToggleSalesforce: () => void; onAssemble: () => void }) {
  return <section className="choose-layout choose-stage" aria-label="Choose deployment">
    <article className="task-surface choose-card">
      <header className="task-heading"><div><h2>Choose a solution</h2><p>Select a quickstart and the systems it should configure.</p></div></header>
      ...
      <section className="system-selection" aria-labelledby="systems-title"><header><h3 id="systems-title">Systems</h3><p>Box is required. Add Salesforce when the solution includes CRM metadata.</p></header><div className="system-grid"><div className="system-option required"><span><strong>Box</strong><small>Content platform</small></span><ProviderSwitch checked disabled /><em>Required</em></div><div className="system-option"><ProviderSwitch checked={selectedComponents.includes('salesforce')} disabled={assembling} label="Salesforce" description="CRM records and workflows" onToggle={onToggleSalesforce} /><em>Optional</em></div><div className="system-option unavailable" aria-disabled="true"><span className="system-placeholder" aria-hidden="true"></span><span><strong>Databricks</strong><small>Data intelligence</small></span><em>Coming soon</em></div><div className="system-option unavailable" aria-disabled="true"><span className="system-placeholder" aria-hidden="true"></span><span><strong>Amazon Bedrock</strong><small>AgentCore integration</small></span><em>Coming soon</em></div></div></section>
      <footer className="choose-actions"><p className="notice" role="status">{notice}</p><div className="stage-navigation"><box-button label={assembling ? 'Preparing package…' : 'Prepare package'} tone="primary" disabled={assembling} onClick={onAssemble}></box-button></div></footer>
    </article>
  </section>
}
