...
export function DeployPage({ plan, run, events, notice, onApply, onDiagnostics }: { plan: DeploymentPlan; run: DispatchRun | null; events: RunEvent[]; notice: string; onApply: () => void; onDiagnostics: (runID: string) => void }) {
  ...
  const statusCopy = status === 'In progress' ? 'Provider progress updates automatically.' : status === 'Complete' ? 'All selected systems finished successfully.' : 'The run stopped before every selected system completed.'

  return <section className="deploy-workspace" aria-label="Live deployment progress">
    <section className="validation-surface">
      <header className="validation-heading run-heading"><div><h2>{title}</h2><p>{statusCopy}</p></div><box-badge label={status} tone={state === 'completed' ? 'success' : state === 'failed' ? 'danger' : 'info'}></box-badge></header>
      <div className="run-overall-progress"><box-progress-bar label={`${completedProviders} of ${providers.length} systems complete`} value={completedProviders} max={providers.length}></box-progress-bar><span>{events.length} update{events.length === 1 ? '' : 's'}</span></div>
      <RunTimeline providers={providers}/>
      {notice && <p className="notice" role="status">{notice}</p>}
      <footer className="action-row">{run?.action === 'validate' && run.status === 'completed' && <box-button label="Apply validated changes" tone="primary" onClick={onApply}></box-button>}{run?.status === 'failed' && <box-button label="View diagnostic guidance" tone="secondary" onClick={() => onDiagnostics(run.id)}></box-button>}</footer>
    </section>
    <DetailsRail title="Run details" description={latestEvent?.message || 'Waiting for provider activity.'}><DetailList rows={[["Status", status], ['Run ID', run?.id || 'Preparing'], ['Current system', currentProvider?.name || (state === 'completed' ? 'All systems complete' : 'Waiting')], ['Completed', `${completedProviders} of ${providers.length}`], ['Strategy', plan.strategy === 'reuse' ? 'Reuse existing' : 'Create new']]}/></DetailsRail>
  </section>
}
