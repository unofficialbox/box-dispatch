...
func deployBoxPrivateAdapters(root string, manifest solution.Manifest, settings solution.BoxDeploymentSettings, selection solution.ComponentSelection, deployable []string, workspaceID string, existing []ResourceReference) ([]string, []ResourceReference, error) {
	...
	// Each surface reports its own outcome, so one failing does not discard the
	// other's result and leave a created surface unrecorded.
	deployed := []string{}
	...
	if request.Form != nil {
		...
		case response.Result.Form == nil:
			issues = append(issues, component+": the browser returned no result")
		case strings.HasPrefix(response.Result.Form.Outcome, "failed"):
			issues = append(issues, component+": "+response.Result.Form.Outcome)
		default:
	...
	if request.App != nil {
		...
		case response.Result.App == nil:
			issues = append(issues, component+": the browser returned no result")
		case strings.HasPrefix(response.Result.App.Outcome, "failed"):
			issues = append(issues, component+": "+response.Result.App.Outcome)
		default:
			deployed = append(deployed, component)
	...
	if len(issues) > 0 {
		return deployed, resources, fmt.Errorf("%s", strings.Join(issues, "; "))
	}
