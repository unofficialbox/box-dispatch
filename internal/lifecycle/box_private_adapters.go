...
func executeBoxPrivateBrowser(request boxPrivateRequest) (boxPrivateResponse, error) {
	...
	tempFile, err := os.CreateTemp("", "windlass-box-private-*.js")
	if err != nil {
		return boxPrivateResponse{}, err
	}
...
repeat with windowIndex from 1 to count of windows
  repeat with tabIndex from 1 to count of tabs of window windowIndex
    set tabURL to URL of tab tabIndex of window windowIndex
    if tabURL contains ".ent.box.com" then return execute tab tabIndex of window windowIndex javascript "JSON.stringify(window.__windlassPrivateResult)"
  end repeat
end repeat
...
func writeBoxPrivateCapture(result []byte) error {
	...
	directory := filepath.Join(home, "Library", "Application Support", "box-windlass", "network-captures")
...
func boxPrivateBrowserScript(payload string) string {
	return fmt.Sprintf(`window.__windlassPrivateResult=null;window.__windlassPrivateNetwork=[];(async()=>{
"use strict";
const request=%s;
if(!location.hostname.endsWith(".box.com"))throw new Error("Windlass private API guard requires an authenticated *.box.com tab");
const network=window.__windlassPrivateNetwork;
...
})().then(value=>{window.__windlassPrivateResult=value}).catch(error=>{window.__windlassPrivateResult={ok:false,error:String(error?.stack||error),network:window.__windlassPrivateNetwork}});`, payload)
}
